...
public class PolicyService
{
    ...
    public async Task<ServiceResponse> GetPolicy()
    {
        ...
        var delimiter = '-';
        var nameComponents = await _fileReader.GetList<ResourceComponent>();
        var resourceTypes = await _fileReader.GetList<ResourceType>();
        var unitDepts = await _fileReader.GetList<ResourceUnitDept>();
        var environments = await _fileReader.GetList<ResourceEnvironment>();
        var locations = await _fileReader.GetList<ResourceLocation>();
        var orgs = await _fileReader.GetList<ResourceOrg>();
        var functions = await _fileReader.GetList<ResourceFunction>();
        var projectAppSvcs = await _fileReader.GetList<ResourceProjAppSvc>();
        ...
        {
            PolicyRule = PolicyRuleFactory.GetNameValidationRules(
                validations.Select(x => new PolicyRule(x, delimiter)).ToList(),
                delimiter)
        };
