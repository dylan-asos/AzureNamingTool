...
public class ResourceNamingRequestService
{
    ...
    public async Task<ResourceNameResponse> RequestNameWithComponents(ResourceNameRequestWithComponents request)
    {
        ...
        // Get the components
        var serviceResponse = await _resourceComponentService.GetItems(false);
        ...
        if (currentResourceComponents != null)
        {
            foreach (var component in currentResourceComponents!)
            {
                ...
                if (!resourceType.Exclude.ToLower().Split(',').Contains(normalizedcomponentname))
                {
                    ...
                    if (prop != null)
                    {
                        if (component.Name == ComponentNames.ResourceInstance)
                        {
                            value = prop;
                        }
                        else
                        ...
                        if (component.Name == ComponentNames.ResourceType)
                        {
        ...
        if (lstComponents.FirstOrDefault(x => x[0] == ComponentNames.ResourceInstance) != null)
        {
            if (lstComponents.FirstOrDefault(x => x[0] == ComponentNames.ResourceInstance)![1] != null)
            {
                if (!ValidationHelper.CheckNumeric(
                        lstComponents.FirstOrDefault(x => x[0] == ComponentNames.ResourceInstance)![1]))
                {
        ...
        var validationResponse = await DoValidationRequest(resourceType, name);
        valid = validationResponse.Valid;
        if (!string.IsNullOrEmpty(validationResponse.Name))
        {
        ...
        {
            sbMessage.Append(validationResponse.Message);
        }
    ...
    private async Task<ValidationResponse> DoValidationRequest(ResourceType resourceType, string name)
    {
        ...
        var validationResponse = await _resourceTypeService.ValidateResourceTypeName(validateNameRequest);
        if (!validationResponse.Success)
            return response;

        if (validationResponse.ResponseObject == null)
            return response;

        var validateNameResponse = (ValidateNameResponse) validationResponse.ResponseObject!;
        response.Valid = validateNameResponse.Valid;

        if (!string.IsNullOrEmpty(validateNameResponse.Name))
        {
            response.Name = validateNameResponse.Name;
        }

        if (!string.IsNullOrEmpty(validateNameResponse.Message))
        {
            response.Message = validateNameResponse.Message;
        }
    ...
    public async Task<ResourceNameResponse> RequestName(ResourceNameRequest request)
    {
        ...
        switch (resourceTypesByShortName.Count)
        {
            case 0:
            ...
            case > 1 when request.ResourceId != 0:
            {
                    ...
                    foreach (var component in currentResourceComponents!)
                    {
                            ...
                            if (!resourceType.Exclude.ToLower().Split(',').Contains(normalizedComponentName))
                            {
                                ...
                                if (value != null)
                                {
                                    if (!string.IsNullOrEmpty(value))
                                    {
                                        ...
                                        {
                                            case ComponentNames.AsLowerCase.ResourceTypeLowerCase:
                                                ...
                                                break;
                                            case ComponentNames.AsLowerCase.ResourceEnvironmentLowerCase:
                                                var environments =
                                                ...
                                                break;
                                            case ComponentNames.AsLowerCase.ResourceLocationLowerCase:
                                                var locations =
                                                ...
                                                break;
                                            case ComponentNames.AsLowerCase.ResourceOrgLowerCase:
                                                ...
                                                break;
                                            case ComponentNames.AsLowerCase.ResourceProjAppSvcLowerCase:
                                                var projappsvcs =
                                                ...
                                                break;
                                            case ComponentNames.AsLowerCase.ResourceUnitDeptLowerCase:
                                                var unitdepts =
                                                ...
                                                break;
                                            case ComponentNames.AsLowerCase.ResourceFunctionLowerCase:
                                                var functions =
                        ...
                        else
                        {
                            if (!component.IsFreeText)
                            {
                                ...
                                var customComponentServiceResponse =
                                    (List<CustomComponent>) serviceResponse.ResponseObject!;

                                // Make sure the custom component has values
                                if (customComponentServiceResponse.Any(x =>
                                        x.ParentComponent == normalizedComponentName))
                                {
                                            ...
                                            .Contains(normalizedComponentName))
                                    {
                                        ...
                                        if (request.CustomComponents != null)
                                        {
                                            ...
                                            if (request.CustomComponents.TryGetValue(normalizedComponentName,
                                                    out var componentvalue))
                                            {
